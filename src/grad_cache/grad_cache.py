...
class GradCache:
    ...
    def build_cache(self, *reps: Tensor, first=0.5, **loss_kwargs) -> [List[Tensor], Tensor]:
        """
    ...
    def cache_step(
        ...
        model_inputs[0] = [model_inputs[0], copy.deepcopy(model_inputs[1])] # q, p
        model_inputs[1] = [model_inputs[1], copy.deepcopy(model_inputs[0][0])] # p ,q

        # for idx in range(len(model_inputs)):
        #     print(len(model_inputs[idx][0]))
        #     print(len(model_inputs[idx][1]))
        #     print("\n")
        # return

        # len 2, tuple, 0 is first model, 1 is for second model
        # within idx, is a tuple of two dicts
        for model, x in zip(self.models, model_inputs):
            model_reps, rnd_states = self.forward_no_grad(model, x[0])
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)

            model_reps, rnd_states = self.forward_no_grad(model, x[1])
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)

        cache, loss = self.build_cache(*all_reps, first=True, **loss_kwargs)
