...
class GradCache:
        ...
        if not scorer:
            ...
            for start in range(0, len(model_inputs[0]), chunk_size):
                with random_states[idx]:
                    ...
                    surrogate = torch.dot(reps.flatten(), cached_gradients[0][start:end].flatten())
                    surrogate.backward(retain_graph=True)

                    reps = self.get_reps(p_emb)
                    surrogate = torch.dot(reps.flatten(), cached_gradients[1][start:end].flatten())
                    surrogate.backward()
    ...
    def cache_step(
        ...
        q_emb, p_emb, rnd_states = self.forward_no_grad(self.models[-1], 
                                        all_reps, chunk_size=chunk, third=True) # forward the dual encoder's rep to cross attn model
