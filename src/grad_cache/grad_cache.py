...
class GradCache:
        ...
        if scorer:
            with torch.no_grad():
                for query in model_inputs[0]:
                    ...
                    rnd_states.append(RandContext(*self.get_input_tensors([query, model_inputs[1]])))
                    y = self.model_call(model, [query, model_inputs[1]])
    ...
    def forward_backward(
        ...
        if not scorer:
            ...
            for query, state, gradient in zip(model_inputs[0], random_states, cached_gradients):
                query = query.reshape(1, 768)
                with state:
                    y = self.model_call(model, [query, model_inputs[1]])
                reps = self.get_reps(y)
    ...
    def cache_step(
        ...
        for model, x in zip(self.models[:2], model_inputs):
            model_reps, rnd_states = self.forward_no_grad(model, x, scorer=False)
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)

        # forward the dual encoder's rep to scorer model
        model_reps, rnd_states = self.forward_no_grad(self.models[-1], all_reps, scorer=True)
        all_reps.append(model_reps)
        all_rnd_states.append(rnd_states)
        
        # using all three reps, calculate loss
        cache, loss = self.build_cache(*all_reps, **loss_kwargs)     
        num_queries = len(model_inputs[0]) * self.chunk_sizes[0]   
        last = cache[-1]
        last = torch.split(last, num_queries*2) # shape of queries by passages
        ...
        self.forward_backward(self.models[-1], all_reps, last, all_rnd_states[-1], no_sync_except_last=no_sync_except_last, scorer=True)
