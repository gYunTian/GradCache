...
class GradCache:
    ...
    def forward_no_grad(
        ...
        if third:
            ...
            q_reps = []
            with torch.no_grad():
                for query in model_inputs[0]:
                    query = query.reshape(1, 768)
                    for passage in model_inputs[1]:
                        passage = passage.reshape(1, 768)
                        rnd_states.append(RandContext(*self.get_input_tensors([query, passage])))
                        p_emb, q_emb = self.model_call(model, [query, passage])
    ...
    def build_cache(self, num_queries: int, reps: Tensor) -> [List[Tensor], Tensor]:
        ...
        with autocast() if self.fp16 else nullcontext():
            loss = self.compute_loss(num_queries=num_queries, x2=reps[0], y2=reps[1])
    ...
    def forward_backward(
        ...
        if not scorer:
            ...
            # need to back prop for both inputs
            # means get q_emb and p_emb
            # then perform backward twice

            # RND STATE: every pair, SHAPE: 32
            # CACHED GRADIENT: for Query & Passage, Each 32, 768
            cached_gradients[0] = torch.squeeze(cached_gradients[0])
            cached_gradients[1] = torch.squeeze(cached_gradients[1])

            length = model_inputs[0].size(0)
            for idx in range(length): # loop every pair
                query = model_inputs[0][idx].reshape(1, 768)
                passage = model_inputs[1][idx].reshape(1, 768)
                with random_states[idx]:
                    p_emb, q_emb  = self.model_call(model, [query, passage])
                reps = self.get_reps(p_emb)
                surrogate = torch.dot(reps.flatten(), cached_gradients[1][idx].flatten())
                surrogate.backward(retain_graph=True)

                reps = self.get_reps(q_emb)
                surrogate = torch.dot(reps.flatten(), cached_gradients[0][idx].flatten())
                surrogate.backward()
    ...
    def cache_step(
        ...
        # Forward for the dual encoders first to get rep
        ...
        # forward the dual encoder's rep to cross attn model
        q_emb, p_emb, rnd_states = self.forward_no_grad(self.models[-1], all_reps, third=True)
        # all_reps.append(q_emb) # order query, passage, query2, passage2
        # all_reps.append(p_emb)
        # all_rnd_states.append(rnd_states)

        num_queries = all_reps[0].size(0)
        ...
        self.forward_backward(
            self.models[-1], all_reps, cache, rnd_states, 
            no_sync_except_last=no_sync_except_last, scorer=True)
